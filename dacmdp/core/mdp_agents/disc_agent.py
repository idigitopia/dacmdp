...
def discount_reward(reward, dist, penalty_beta, penalty_type="linear"):
    """
...
# def calc_prob_distr(knn_dist_dict, delta, uniform_distr=False):
#     """
...
#         all_knn_sim_scores = {nn: 1 / (dist + delta) for nn, dist in knn_dist_dict.items()}
...
class DACAgentBase(object):
    ...
    def __init__(self, action_space, seed_mdp, repr_model, build_args, solve_args, eval_args):
        ...
        assert has_attributes(repr_model, ["encode_state_action_batch", "encode_action_single", "encode_obs_batch",
        ...
        self.tt2i, self.i2tt = {tt: i for i, tt in enumerate(self.tran_types)}, {
            i: tt for i, tt in enumerate(self.tran_types)}
        ...
        #         s-2  .
        #         s-3  .
        # Initialize after state KD Tree build, State transition type to idx matrix
        self.stt2a_idx_matrix = None

        # cache for fishing out variables for logging purposes.
        self.cache = {}
        # cache a pointer to the buffer to be processed.
    ...
    def _batch_parse(self, obs_batch: torch.tensor, a_batch: torch.tensor, obs_prime_batch: torch.tensor, r_batch: torch.tensor, d_batch: torch.tensor):
        ...
        s_batch, s_prime_batch = map(self.repr_model.encode_obs_batch, [
                                     obs_batch, obs_prime_batch])
    ...
    def parse_all_transitions(self, buffer, _batch_size=256):
        ...
        for idxs in batch_iterator:
            ...
            batch_d = 1 - batch_nd
            self._batch_parse(batch_ob, batch_a, batch_ob_prime,
                              batch_r.view((-1,)), batch_d.view((-1,)))
        ...
        self.v_print("Step 1 [Parse Transitions]:  Complete,  Time Elapsed: {}\n\n".format(
            time.time() - st))
        self.v_print("length of to parse transitions",
                     len(self.parsed_transitions))
    ...
    def build_kdtree(self):
        ...
        self.parsed_unique_states = np.unique(np.stack(
            [s for s, _, _, _, _ in self.parsed_transitions] + [self.end_state_vector]), axis=0)
        ...
        self.parsed_unique_actions = np.unique(
            np.stack([a for s, a, _, _, _ in self.parsed_transitions]), axis=0)
        ...
        self.action_factored_states = {a: [] for a in get_action_list_from_space(
            self.action_space)}  # factored state list
        for s, a, ns, r, d in self.parsed_transitions:
    ...
    def compute_dac_dynamics(self):
        ...
        self.stt2a_idx_matrix = np.zeros(
            (len(self.s_kdTree.s2i), self.build_args.tran_type_count)).astype(np.int32)
        ...
        for tt, cand_a in zip(self.tran_types, get_action_list_from_space(self.action_space)):
            for s_batch in v_iter(iter_batch(all_states, _batch_size), self._verbose,
                ...
                knn_output = self.a_s_kdTrees[cand_a].get_knn_batch(
                    s_batch, self.build_args.mdp_build_k)
                all_sa_nn.update(
                    {(state, cand_a): knn_dict for state, knn_dict in zip(s_batch, knn_output)})
        ...
        for i, tran in v_iter(enumerate(self.parsed_transitions), self._verbose,  "Calculating DAC Dynamics"):
            ...
            for tt, cand_a in zip(self.tran_types, get_action_list_from_space(self.action_space)):
                self.stt2a_idx_matrix[self.s_kdTree.s2i[s]
                                      ][self.tt2i[tt]] = self.a_kdTree.s2i[cand_a]
                knn_cand_sa = all_sa_nn[(s, cand_a)]
                knn_cand_sa_normalized = MyKDTree.calc_prob_distr(knn_cand_sa, delta=self.build_args.knn_delta,
                                                                  force_uniform_distr=True if not self.build_args.normalize_by_distance else False)
                ...
                for (nn_s, nn_d), (_, nn_prob) in zip(knn_cand_sa.items(), knn_cand_sa_normalized.items()):
                    ...
                    count_ = int(
                        nn_d * 100) if self.build_args.normalize_by_distance else 1
                    self.tC[s][tt][nn_ns] += count_
                    self.rC[s][tt][nn_ns] += expec_r * count_
                    self.cC[s][tt][nn_ns] += cost * count_
    ...
    def initialize_MDP(self):
        ...
        self.v_print("Step 4 [Initialize MDP]MM: Running")
        st = time.time()
            ...
            for tt in self.tC[s]:
                for slot, ns in enumerate(self.tC[s][tt]):
                    ...
                    if self.mdp_T.is_factored:
                        self.mdp_T.update_count_matrices(
                            s_i, tt_i, ns_i, r_sum=r_sum, c_sum=c_sum, count=tran_count, slot=slot, append=False)
                    else:
                        self.mdp_T.update_count_matrices(
                            s_i, tt_i, ns_i, r_sum=r_sum - c_sum, count=tran_count, slot=slot, append=False)

                self.mdp_T.update_prob_matrices(s_i, tt_i)

        self.v_print("Step 4 [Initialize MDP]: Complete,  Time Elapsed: {}".format(
            time.time() - st))
        self.v_print(
            f"Total index processed:{idx_found + idx_missing}, Success: {idx_found} Failure: {idx_missing} | {100*idx_missing/(idx_found + idx_missing):.4f}% \n\n")

    # step 5
    def solve_mdp(self):
        """ Solves the internal MDP object """
        self.v_print("Step 5 [Solve MDP]:  Running")
        st = time.time()

        self.mdp_T.curr_vi_error = 10
        self.mdp_T.solve(eps=0.001, mode="GPU",
                         safe_bkp=True, verbose=self.verbose)
    ...
    def process(self, train_buffer, match_hash=True):
        ...
        # Step 1 : Pre-Compute all embeddings and KD Tree necessary.
        self.parse_all_transitions(train_buffer)
        time.sleep(1)
        self.build_kdtree()
        time.sleep(1)

        # Step 2 : Compute - Build - then - Solve the DAC - MDP
        if self.build_args.rebuild_mdpfcache:
            self.v_print(
                "Rebuilding MDP: loading Cached solution Vectors from", self.build_args.save_folder)
            self.load_sol_vectors(
                self.build_args.save_folder, match_hash=match_hash)
        else:
            self.compute_dac_dynamics()
            time.sleep(1)
            self.initialize_MDP()
            time.sleep(1)
            self.solve_mdp()
        ...
        if self.build_args.save_mdp2cache:
            self.v_print("Caching MDP: Storing solution Vectors to",
                         self.build_args.save_folder)
    ...
    def opt_policy(self, obs):
        return self.calc_eval_policy(self.repr_model.encode_obs_single(obs), self.mdp_T.qD_cpu,
    ...
    def safe_policy(self, obs):
        return self.calc_eval_policy(self.repr_model.encode_obs_single(obs), self.mdp_T.s_qD_cpu,
                                     soft=self.eval_args.soft_at_plcy,
                                     weight_nn=self.build_args.normalize_by_distance,
                                     plcy_k=self.eval_args.plcy_k)
    ...
    def calc_eval_policy(self, s, qMatrix, soft=False, weight_nn=False, plcy_k=1):
        """
        Fetches the optimal action from the q_matrix for the query state. 
        ToDo this needs some additional work. 
        """
        # for all actions in candidate actions
        # calcualate state action distribution
        # calculate knn neighboring state action pairs.
        # fetch the q value of the knn neighboring state action pairs
        # weight the q values according to the distances to state action pairs.

        knn_s = self.s_kdTree.get_knn(s, k=plcy_k)
        knn_s_norm = MyKDTree.calc_prob_distr(knn_s, delta=self.build_args.knn_delta) \
        ...
        for s, nn_d in knn_s.items():
            for tt in self.tran_types:
                ...
                # get action index from (s_i,tt_i) to a_i map
                a_i = self.stt2a_idx_matrix[s_i][tt_i]
                a_vec = self.a_kdTree.i2s[a_i]  # get action vector.

                cost = cost_logic(nn_d, self.build_args.penalty_beta)
                qval_dict[a_vec].append(qMatrix[s_i, tt_i] - cost)

        avg_qval_dict = {a_vec: np.mean(vals)
                         for a_vec, vals in qval_dict.items()}
        ...
        self.cache["q_matrix_query"] = avg_qval_dict
    ...
    def cache_sol_vectors(self, save_folder):
        ...
        mdp_hmap = {a: dict_hash(getattr(self.mdp_T, a))
                    for a in mdp_hash_attrs}
    ...
    def load_sol_vectors(self, save_folder, match_hash=True):
        ...
        if match_hash:
            ...
            print(
                "Warning Hash map of indexes are not being check, NN might be different")
