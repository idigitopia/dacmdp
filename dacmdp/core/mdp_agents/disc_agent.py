...
class DACAgentBase(object):
    ...
    def __init__(self, seed_mdp, repr_model, build_args, solve_args , eval_args):
...
#         self.action_vec_size = action_space.shape[0]
        ...
        self.tran_types = seed_mdp.A
        self.tt2i, self.i2tt = {tt:i for i,tt in enumerate(self.tran_types)}, {i:tt for i,tt in enumerate(self.tran_types)}
        self.stt2a_idx_matrix = None # Initialize after state KD Tree build, State transition type to idx matrxi
    ...
    def build_kdtree(self):
        ...
        for s, a, ns, r, d in self. in self.parsed_transitions:
            self.tree_f_states[a].append(s)
                
        self.a_s_kdTrees = {a: MyKDTree(st_list) 
                            for a, st_list in self.tree_f_states.items()
                            if len(st_list) > 1}
    ...
    def intialize_dac_dynamics(self):
        ...
        for s_batch in v_iter(iter_batch(all_states, _batch_size),self._verbose, "Calculate KNN for all parsed States"):
            for a in self.tran_types:
                knn_output = self.s_kdTree.get_knn_batch(s_batch, self.build_args.tran_type_count)
                all_nn.update({s:knn_dict for s,knn_dict in zip(s_batch, knn_output)})
            ...
            for a in self.tC[s]:
                for slot, ns in enumerate(self.tC[s][a]):
                    ...
                    try:
                        ...
                        # Get Counts
                        tran_count, r_sum, c_sum  = self.tC[s][a][ns], self.rC[s][a][ns], self.cC[s][a][ns]
